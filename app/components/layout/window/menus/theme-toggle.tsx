...
import { Theme, useTheme } from "~/lib/providers/theme-provider";
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <DropdownMenuTrigger asChild>
            <Button
              className={cn(
                "group opacity-50 transition-opacity group-hover/title-bar:opacity-100 size-[1.625rem]",
                className,
              )}
              variant={"ghost"}
              size={"icon"}
      ...
      <DropdownMenuContent align="center" sideOffset={6}>
        {[
          ...
          >
            ...
            {titleCase(theme)}
            {activeTheme === theme && (
              <Check className="ml-auto mr-1 stroke-green-500" size={14} />
            )}
          </DropdownMenuItem>
        ))}
        <DropdownMenuSeparator />
